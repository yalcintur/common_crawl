...
class DocumentProcessor:
    ...
    def process_doc_worker(self, doc):
        # Process each document in a separate process
    ...
    def process_result(self, result):
        ...
        if is_duplicate:
            self.duplicates.add(f"doc_{self.doc_count}")
    ...
    def run(self):
        with ThreadPoolExecutor(max_workers=os.cpu_count()) as executor:
            ...
            while True:
                ...
                if doc is None:
                    if batch:
                        futures.append(executor.submit(self.process_doc_batch, batch))
                    break
            ...
            for future in as_completed(futures):
                for doc, is_duplicate, is_repetitive in future.result():
                    self.process_result(doc, is_duplicate, is_repetitive)
...
def deduplicate_and_process(docs_queue, clean_docs_queue, num_perm=128, threshold=0.6):
    doc_processor = DocumentProcessor(docs_queue, clean_docs_queue, num_perm, threshold)
    ...
    while True:
        ...
        processed_result = doc_processor.process_doc_worker(doc)
        doc_processor.process_result(processed_result)

    # Signal the end of processing to the writer thread
    clean_docs_queue.put(None)

def writer_thread(clean_docs_queue, output_dir, batch_size=300000):
    batch = []
    batch_count = 0
    ...
    while True:
        ...
        if clean_doc is None:
            ...
            if batch:
                with tqdm(
                    total=len(batch), desc=f"Writing Batch {batch_count}"
                ) as pbar:
                    write_parquet(
                        batch, output_dir, f"clean_docs_batch_{batch_count}", len(batch)
                    )
...
def main():
    ...
    docs_queue = Queue(maxsize=2000000)
    clean_docs_queue = Queue()

    print("Dedup thread has started")
    dedup_thread = Thread(
        target=deduplicate_and_process, args=(docs_queue, clean_docs_queue)
    )
    dedup_thread.start()

    print("Writer thread has started")
    writer_thread_instance = Thread(
        target=writer_thread, args=(clean_docs_queue, args.output, 300)
    )
